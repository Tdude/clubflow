...
final class ClubFlow_Ajax {
	...
	/**
	 * TEMPORARY diagnostic for the empty-calendar bug. Admin-only. Remove after use.
	 */
	private function emit_events_debug(): void {
		global $wpdb;
		$out = [];
		$out['plugin_version']   = ClubFlow::VERSION;
		$out['wp_version']       = get_bloginfo('version');
		$out['php_version']      = PHP_VERSION;
		$out['session_sql_mode'] = $wpdb->get_var('SELECT @@SESSION.sql_mode');
		$out['global_sql_mode']  = $wpdb->get_var('SELECT @@GLOBAL.sql_mode');
		$out['posts_clauses_hooked'] = has_filter('posts_clauses') ? 'yes' : 'no';
		$out['ext_object_cache'] = wp_using_ext_object_cache() ? 'yes' : 'no';

		// Raw postmeta presence (bypasses WP_Query entirely)
		$out['postmeta__clubflow_start_rows'] = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key = '_clubflow_start'");
		$out['postmeta__clubflow_event_mode_rows'] = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key = '_clubflow_event_mode'");
		$out['event_mode_value_distribution'] = $wpdb->get_results("SELECT meta_value, COUNT(*) c FROM {$wpdb->postmeta} WHERE meta_key='_clubflow_event_mode' GROUP BY meta_value", ARRAY_A);

		// Plain query (no meta_query) — mirrors the working REST query
		$wpdb->last_error = '';
		$plain = new \WP_Query(['post_type' => ClubFlow::POST_TYPE, 'post_status' => ['publish', 'future'], 'posts_per_page' => 1, 'fields' => 'ids']);
		$out['plain_query_found']       = (int) $plain->found_posts;
		$out['plain_query_last_error']  = $wpdb->last_error;

		// meta EXISTS only
		$wpdb->last_error = '';
		$mex = new \WP_Query(['post_type' => ClubFlow::POST_TYPE, 'post_status' => ['publish', 'future'], 'posts_per_page' => 1, 'fields' => 'ids', 'meta_query' => [['key' => '_clubflow_start', 'compare' => 'EXISTS']]]);
		$out['start_exists_found']      = (int) $mex->found_posts;
		$out['start_exists_last_error'] = $wpdb->last_error;

		// The actual calendar query
		$wpdb->last_error = '';
		$cal = new \WP_Query([
			'post_type' => ClubFlow::POST_TYPE, 'post_status' => ['publish', 'future'], 'posts_per_page' => 500,
			'orderby' => 'meta_value', 'meta_key' => '_clubflow_start', 'order' => 'ASC',
			'meta_query' => ['relation' => 'AND',
				['key' => '_clubflow_start', 'compare' => 'EXISTS'],
				['relation' => 'OR', ['key' => '_clubflow_event_mode', 'value' => 'calendar'], ['key' => '_clubflow_event_mode', 'compare' => 'NOT EXISTS']],
			],
		]);
		$out['calendar_query_count']      = count($cal->posts);
		$out['calendar_query_last_error'] = $wpdb->last_error;
		$out['calendar_query_sql']        = $cal->request;

		// Same query but bypassing the object cache, to expose stale cached-empty results
		$wpdb->last_error = '';
		$cal_uncached = new \WP_Query([
			'post_type' => ClubFlow::POST_TYPE, 'post_status' => ['publish', 'future'], 'posts_per_page' => 500,
			'orderby' => 'meta_value', 'meta_key' => '_clubflow_start', 'order' => 'ASC', 'cache_results' => false,
			'meta_query' => ['relation' => 'AND',
				['key' => '_clubflow_start', 'compare' => 'EXISTS'],
				['relation' => 'OR', ['key' => '_clubflow_event_mode', 'value' => 'calendar'], ['key' => '_clubflow_event_mode', 'compare' => 'NOT EXISTS']],
			],
		]);
		$out['calendar_query_count_uncached']      = count($cal_uncached->posts);
		$out['calendar_query_uncached_last_error'] = $wpdb->last_error;

		// Sample of newest published events: meta as get_post_meta sees it live
		$sample = get_posts(['post_type' => ClubFlow::POST_TYPE, 'post_status' => 'publish', 'numberposts' => 3, 'orderby' => 'ID', 'order' => 'DESC']);
		$out['sample_events'] = [];
		foreach ($sample as $p) {
			$out['sample_events'][] = [
				'id'    => $p->ID,
				'start' => get_post_meta($p->ID, '_clubflow_start', true),
				'end'   => get_post_meta($p->ID, '_clubflow_end', true),
				'mode'  => get_post_meta($p->ID, '_clubflow_event_mode', true),
			];
		}
	...
	public function ajax_events(): void {
		// TEMP diagnostic (admins only, no nonce needed): open while logged in:
		//   /wp-admin/admin-ajax.php?action=clubflow_events&clubflow_debug=1
		if (isset($_GET['clubflow_debug']) && is_user_logged_in() && current_user_can('manage_options')) {
			$this->emit_events_debug();
			return;
		}
